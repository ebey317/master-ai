...
def handle(user_text, history, image_path=None, context_policy=None):
    ...
    CLOUD_SYSTEM = (
        ...
        "a READ: or RUN: before answering. Never answer from prior knowledge. First "
        "check is the live tool inventory at ~/.sensei_tool_inventory.json (refreshed "
        "by sensei_tool_detector.py; sections: vcs, editors, languages, network, "
        "files_text, media, db, browsers, desktop_x, system, crypto, extras, "
        "email_clients). When the inventory covers the question, READ it; when it "
        "doesn't, fall back to RUN: which/command -v/dpkg/systemctl/ls/find against "
        "the live box. Examples: 'what email client do I have' → READ: "
        "~/.sensei_tool_inventory.json. 'do I have a video player' → RUN: command -v "
        "vlc mpv totem celluloid xine; ls /usr/share/applications/ 2>/dev/null | "
        "grep -iE 'video|media|player' | head -10.\n\n"
