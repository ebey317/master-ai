...
def _api_prompt(prompt, *, source="", page_context=None, schedule_id="",
    ...
    lines.extend([
        ...
        "Browser lane — for in-tab work, emit ONLY these nine directives: BROWSER_CLICK, BROWSER_DOUBLE_CLICK, BROWSER_FILL, BROWSER_SUBMIT, BROWSER_NAV, BROWSER_WAIT, BROWSER_SCROLL, BROWSER_READ_PAGE, BROWSER_SCREENSHOT. The extension confirms and dispatches them in-tab. BROWSER_DOUBLE_CLICK is the right tool for Drive search-result rows and other UIs where single-click only selects. Do NOT emit BROWSER_DRIVE_INSPECT_FOLDER, BROWSER_FIND, BROWSER_EXTRACT_LIST, BROWSER_OBSERVE, BROWSER_READ, BROWSER_CLOSE_TAB, BROWSER_TAB_CREATE, BROWSER_RESIZE_WINDOW, BROWSER_CDP_MOUSE, BROWSER_CDP_KEY, BROWSER_JS, BROWSER_CONSOLE, or BROWSER_NETWORK — those exist for the Pupil web UI but the chrome_extension cannot dispatch them. For tasks they would have served (listing folder contents, finding a label, extracting a list, deeply inspecting page state) fall back to BROWSER_READ_PAGE on a navigated tab and parse the returned DOM, or use the terminal lane (RUN with a Drive API call, etc.).",
